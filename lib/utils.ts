import { Schema, ValidatedSchema } from './schema';
...
export class TransformError extends Error {
    constructor(readonly msg: string, public readonly args: unknown) {
        super(msg);
 ...
 * @private
 */
export function validateSchema({ name, schema = {} }: { name: string; schema?: Schema }): ValidatedSchema {
    if (!isObject(schema)) {
    ...
    if (!isFunction(schema.data.id)) {
        schema.data.id = ({ data }: { data: any }) => data.id.toString();
    }
    ...
    if (schema.data.relationships) {
        if (!isObject(schema.data.relationships)) {
            ...
            for (const rel of Object.keys(schema.data.relationships)) {
    ...
    return schema as ValidatedSchema;
}
 ...
 * @private
 */
export function validateJsonApiDocument(document: JsonapiDocument) {
    ...
    if (document.data) {
        ...
        resources.forEach((resource: any) => {
            ...
            if (resource.attributes && !isObject(resource.attributes)) {
                throw new Error(
                    `Primary data resource "${resource.id}, ${resource.type}" field "attributes" must be an object`
                );
            }
            ...
            if (resource.relationships) {
                if (!isObject(resource.relationships)) {
                    throw new Error(
                        `Primary data resource "${resource.id}, ${resource.type}" field "relationships" must be an object`
                    );
                }
                ...
                Object.keys(resource.relationships).forEach((relationshipName) => {
                    ...
                    if (!relationship.data) {
                        throw new Error(
                            `Relationship "${relationshipName}" of primary data resource "${resource.id}, ${resource.type}" must have a "data" field`
                        );
                    }
                    ...
                    data.forEach((d: any) => {
                        if (!d.id) {
                            throw new Error(
                                `Data of relationship "${relationshipName}" of primary data resource "${resource.id}, ${resource.type}" must have an "id" field`
                            );
                        }

                        if (!isString(d.id)) {
                            throw new Error(
                                `Data "${d.id}" of relationship "${relationshipName}" of primary data resource "${resource.id}, ${resource.type}" must be a string`
                            );
                        }

                        if (!d.type) {
                            throw new Error(
                                `Data "${d.id}" of relationship "${relationshipName}" of primary data resource "${resource.id}, ${resource.type}" must have a "type" field`
                            );
                        }

                        if (!isString(d.type)) {
                            throw new Error(
                                `Type "${d.type}" of relationship "${relationshipName}" of primary data resource "${resource.id}, ${resource.type}" must be a string`
                            );
                        }
                    });
                });
    ...
    if (document.included) {
        ...
        document.included.forEach((resource: any) => {
            ...
            if (resource.attributes && !isObject(resource.attributes)) {
                throw new Error(
                    `Included data resource "${resource.id}, ${resource.type}" field "attributes" must be an object`
                );
            }
            ...
            if (resource.relationships) {
                if (!isObject(resource.relationships)) {
                    throw new Error(
                        `Included data resource "${resource.id}, ${resource.type}" field "relationships" must be an object`
                    );
                }
                ...
                Object.keys(resource.relationships).forEach((relationshipName) => {
                    ...
                    if (!relationship.data) {
                        throw new Error(
                            `Relationship "${relationshipName}" of included data resource "${resource.id}, ${resource.type}" must have a "data" field`
                        );
                    }
                    ...
                    data.forEach((d: any) => {
                        if (!d.id) {
                            throw new Error(
                                `Data of relationship "${relationshipName}" of included data resource "${resource.id}, ${resource.type}" must have an "id" field`
                            );
                        }

                        if (!isString(d.id)) {
                            throw new Error(
                                `Data "${d.id}" of relationship "${relationshipName}" of included data resource "${resource.id}, ${resource.type}" must be a string`
                            );
                        }

                        if (!d.type) {
                            throw new Error(
                                `Data "${d.id}" of relationship "${relationshipName}" of included data resource "${resource.id}, ${resource.type}" must have a "type" field`
                            );
                        }

                        if (!isString(d.type)) {
                            throw new Error(
                                `Type "${d.type}" of relationship "${relationshipName}" of included data resource "${resource.id}, ${resource.type}" must be a string`
                            );
                        }
                    });
                });
