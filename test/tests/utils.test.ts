...
import { JsonapiDocument, Schema } from '../../lib';
...
describe('validateSchema', function () {
    it('should validate empty schema', function () {
        ...
        expect(schema.data.type({} as any)).to.equal('test');
        expect(schema.data.id({ data: { id: 1 } } as any)).to.equal('1');
    });

    it('should fail if schema is not an object', function () {
        const schema = 'foo' as Schema;
        expect(() => validateSchema({ name: 'test', schema })).to.throw('Invalid "schema" Property');
    });

    it('should fail if schema.data.untransformDataSchema exists and is not a function', function () {
        const schema = ({ data: { untransformDataSchema: 'foo' } } as unknown) as Schema;
        expect(() => validateSchema({ name: 'test', schema })).to.throw(
            'Invalid "schema.data.untransformDataSchema" Property'
        );
    });

    it('should fail if schema.data.untransformId exists and is not a function', function () {
        const schema = ({ data: { untransformId: 'foo' } } as unknown) as Schema;
        expect(() => validateSchema({ name: 'test', schema })).to.throw('Invalid "schema.data.untransformId" Property');
    });

    it('should fail if schema.data.links exists and is not a function', function () {
        const schema = ({ data: { links: 'foo' } } as unknown) as Schema;
        expect(() => validateSchema({ name: 'test', schema })).to.throw('Invalid "schema.data.links" Property');
    });

    it('should fail if schema.data.meta exists and is not a function', function () {
        const schema = ({ data: { meta: 'foo' } } as unknown) as Schema;
        expect(() => validateSchema({ name: 'test', schema })).to.throw('Invalid "schema.data.meta" Property');
    });

    it('should fail if schema.data.attributes exists and is not a function', function () {
        const schema = ({ data: { attributes: 'foo' } } as unknown) as Schema;
        expect(() => validateSchema({ name: 'test', schema })).to.throw('Invalid "schema.data.attributes" Property');
    });

    it('should fail if schema.data.untransformAttributes exists and is not a function', function () {
        const schema = ({ data: { untransformAttributes: 'foo' } } as unknown) as Schema;
        expect(() => validateSchema({ name: 'test', schema })).to.throw(
            'Invalid "schema.data.untransformAttributes" Property'
        );
    });

    it('should fail if schema.data.relationships exists and is not an object', function () {
        const schema: Schema = { data: { relationships: (() => {}) as any } };
        expect(() => validateSchema({ name: 'test', schema })).to.throw('Invalid "schema.data.relationships" Property');
    });

    it('should fail if a relationship key exists and its value is not a function', function () {
        const schema: Schema = { data: { relationships: { foo: 'bar' } as any } };
        expect(() => validateSchema({ name: 'test', schema })).to.throw(
            'Invalid Schema: Relationship "foo" should be a function'
        );
    });

    it('should fail if schema.links exists and is not a function', function () {
        const schema: Schema = ({ links: 'foo' } as unknown) as Schema;
        expect(() => validateSchema({ name: 'test', schema })).to.throw('Invalid "schema.links" Property');
    });

    it('should fail if schema.meta exists and is not a function', function () {
        const schema = ({ meta: 'foo' } as unknown) as Schema;
        expect(() => validateSchema({ name: 'test', schema })).to.throw('Invalid "schema.meta" Property');
    });
});
...
describe('validateJsonApiDocument', function () {
    ...
    it('should succeed if the JSON-API document is valid', function () {
        ...
        expect(() => validateJsonApiDocument(document)).to.not.throw();
    });

    it('should fail if the JSON-API document is not an object', function () {
        const document = JSON.stringify(documentTemplate) as JsonapiDocument;
        expect(() => validateJsonApiDocument(document)).to.throw('JSON-API document must be an object');
    });
    ...
    it('should fail if the JSON-API document is missing the primary top level fields', function () {
        ...
        expect(() => validateJsonApiDocument(document)).to.throw(
            'JSON-API document must contain at least one of "data", "errors", or "meta"'
        );
    });
    ...
    it('should fail if the JSON-API document contains both "data" and "errors" fields', function () {
        ...
        expect(() => validateJsonApiDocument(document)).to.throw(
            'JSON-API document must not contain both "data" and "errors"'
        );
    });
    ...
    it('should fail if the JSON-API document contains "included" field but not "data" field', function () {
        ...
        expect(() => validateJsonApiDocument(document)).to.throw(
            'JSON-API document cannot contain "included" without "data"'
        );
    });
    ...
    it('should fail if the JSON-API document primary data resource id is not a string', function () {
        ...
        expect(() => validateJsonApiDocument(document)).to.throw(
            `Primary data resource id "${document.data.id}" must be a string`
        );
    });
    ...
    it('should fail if the JSON-API document primary data resource type is missing', function () {
        ...
        expect(() => validateJsonApiDocument(document)).to.throw(
            `Primary data resource "${document.data.id}" must have a "type" field`
        );
    });
    ...
    it('should fail if the JSON-API document primary data resource type is not a string', function () {
        ...
        expect(() => validateJsonApiDocument(document)).to.throw(
            `Primary data resource type "${document.data.type}" must be a string`
        );
    });
    ...
    it('should fail if the JSON-API document primary data resource attributes is not an object', function () {
        ...
        expect(() => validateJsonApiDocument(document)).to.throw(
            `Primary data resource "${document.data.id}, ${document.data.type}" field "attributes" must be an object`
        );
    });
    ...
    it('should fail if the JSON-API document primary data resource relationships is not an object', function () {
        ...
        expect(() => validateJsonApiDocument(document)).to.throw(
            `Primary data resource "${document.data.id}, ${document.data.type}" field "relationships" must be an object`
        );
    });
    ...
    it('should fail if the JSON-API document primary data resource relationship data is missing', function () {
        ...
        expect(() => validateJsonApiDocument(document)).to.throw(
            `Relationship "author" of primary data resource "${document.data.id}, ${document.data.type}" must have a "data" field`
        );
    });
    ...
    it('should fail if the JSON-API document primary data resource relationship id is missing', function () {
        ...
        expect(() => validateJsonApiDocument(document)).to.throw(
            `Data of relationship "author" of primary data resource "${document.data.id}, ${document.data.type}" must have an "id" field`
        );
    });
    ...
    it('should fail if the JSON-API document primary data resource relationship id is not a string', function () {
        ...
        expect(() => validateJsonApiDocument(document)).to.throw(
            `Data "${document.data.relationships.author.data.id}" of relationship "author" of primary data resource "${document.data.id}, ${document.data.type}" must be a string`
        );
    });
    ...
    it('should fail if the JSON-API document primary data resource relationship type is missing', function () {
        ...
        expect(() => validateJsonApiDocument(document)).to.throw(
            `Data "${document.data.relationships.author.data.id}" of relationship "author" of primary data resource "${document.data.id}, ${document.data.type}" must have a "type" field`
        );
    });
    ...
    it('should fail if the JSON-API document primary data resource relationship type is not a string', function () {
        ...
        expect(() => validateJsonApiDocument(document)).to.throw(
            `Type "${document.data.relationships.author.data.type}" of relationship "author" of primary data resource "${document.data.id}, ${document.data.type}" must be a string`
        );
    });
});
